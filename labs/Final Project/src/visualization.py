...
def plot_product_comparison(self, data: pd.DataFrame,
                             product_ids: List[str]) -> Tuple[plt.Figure, str]:
    """
    Create a radar chart comparing multiple products across impact categories
    and return normalized data as JSON.
    """
    ...
    normalized_json = normalized.to_json(orient='index', indent=4)
    ...
    # Grafiği göstermek için plt.tight_layout() ekleyebilirsiniz
    plt.tight_layout()
    
    return fig, normalized_json
...
def plot_product_comparison(self, data: pd.DataFrame, product_ids: list, save_path: Path = None):
    """
    Create a radar chart comparing multiple products across impact categories.
    """
    import matplotlib.pyplot as plt
    ...
    total_impacts = data[data.index.isin(product_ids)][
        ['carbon_impact', 'energy_impact', 'water_impact', 'waste_generated_kg']
    ]

    # Normalize
    normalized = total_impacts / total_impacts.max()
    ...
    for idx, product_id in enumerate(normalized.index):
