...
class LCACalculator:
    ...
    def get_impact_summary(self, stage_impacts: pd.DataFrame) -> dict:
        """
        Generate summary statistics (min, max, mean) for each impact type.

        Args:
            stage_impacts: DataFrame with stage-level impact data

        Returns:
            Dictionary of impact summaries
        """
        ...
        for impact in impact_types:
            summary[impact] = {
                'min': stage_impacts[impact].min(),
                'max': stage_impacts[impact].max(),
                'mean': stage_impacts[impact].mean()
            }
